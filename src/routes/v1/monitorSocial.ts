...
const HEATMAP_METRICS: SocialHeatmapMetric[] = ["er", "engagement_total", "likes", "comments", "shares", "views", "view_rate"];
...
export const getMonitorSocialHeatmap = async (event: APIGatewayProxyEventV2) => {
  ...
  if (!metric) {
    return json(422, {
      error: "validation_error",
      message: "metric must be er|engagement_total|likes|comments|shares|views|view_rate"
    });
  }
